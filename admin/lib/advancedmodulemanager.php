...
$lang->_load( JPATH_ADMINISTRATOR.'/'.'language'.'/'.'en-GB'.'/'.'en-GB.com_nonumber-installer-uninstallme.ini', 'com_nonumber-installer-uninstallme', 0 );

$install_file = dirname( __FILE__ ).'/'.'extensions.php';
if ( !is_readable ( $install_file ) ) {
	...
	if ( !$has_installed && !$has_updated ) {
		...
		$comp_folder = JPATH_ADMINISTRATOR.'/'.'components'.'/'.'com_nonumber-installer-uninstallme';

		if ( !JFolder::exists( $comp_folder.'/'.'files' ) ) {
			$mainframe->enqueueMessage( sprintf( JText::_( 'Cannot find the required files folder' ), 'files' ), 'message' );
		} else if ( !JFolder::exists( $comp_folder.'/'.'files'.'/'.'forced' ) ) {
			$mainframe->enqueueMessage( sprintf( JText::_( 'Cannot find the required files folder' ), 'files/forced' ), 'message' );
		} else {
			$succes = 1;
			if ( JFolder::exists( $comp_folder.'/'.'files'.'/'.'not_forced' ) && !copy_from_folder( $comp_folder.'/'.'files'.'/'.'not_forced', 0 ) ) {
				$succes = 0;
			}
			if ( !copy_from_folder( $comp_folder.'/'.'files'.'/'.'forced', 1 ) ) {
				$succes = 0;
			}
			...
			if ( $succes ) {
				...
				$mainframe->enqueueMessage( JText::_( 'Please clear your browsers cache' ), 'notice' );
				installElements();
			} else {
...
{
	$install_folder = JPATH_ADMINISTRATOR.'/'.'components'.'/'.'com_nonumber-installer-uninstallme'.'/'.'files'.'/'.'elements';
	$xml_file = $install_folder.'/'.'plugins'.'/'.'system'.'/'.'nonumberelements.xml';
	if ( !JFile::exists( $xml_file) ) {
		return;
	}
	$xml_new = JApplicationHelper::parseXMLInstallFile( $install_folder.'/'.'plugins'.'/'.'system'.'/'.'nonumberelements.xml' );

	$do_install = 1;
	if ( $xml_new && isset( $xml_new['version'] ) ) {
		$do_install = 1;
		$xml_file = JPATH_SITE.'/'.'plugins'.'/'.'system'.'/'.'nonumberelements.xml';
		if ( JFile::exists( $xml_file) ) {
